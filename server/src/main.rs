...
mod tests {
    ...
    async fn single_content() {
        ...
        let view_request = test::TestRequest::get()
            ...
            .to_request();
        let mut view_response: db::models::content::FullContent =
            test::call_and_read_body_json(&app, view_request).await;

        // Sends update request. Changes slug
        view_response
            .base_content
            .set_slug(String::from("new-slug"));
    ...
    async fn list_test() {
        ...
        let mut projects_added: HashMap<usize, db::models::content::NewFullContent> =
            HashMap::new();
        ...
        for _ in 0..CONTENT_AMOUNT {
            ...
            match &add_data.new_extra_content {
                NewExtraContent::Project(_) => {
                    projects_added.insert(projects_added.len(), add_data)
                }
        ...
        let blogs_returned: FullContentList =
            test::call_and_read_body_json(&app, list_request_blog).await;
        let projects_returned_two: FullContentList =
            test::call_and_read_body_json(&app, list_request_project_two).await;
        ...
        assert_eq!(
            projects_returned
                ...
                .get_slug(),
            projects_added
                .get(&(projects_added.len() - 3))
                .unwrap()
                .new_base_content
                .get_slug()
        );
        assert_eq!(
            projects_returned_two
                .get_list()
                .get(2)
                .unwrap()
                .base_content
    ...
    async fn create_test_content(
        add_amount: Option<i32>,
    ) -> HashMap<i32, db::models::content::NewFullContent> {
        let (db_pool, admin_info) = setup_app();
        let app = test::init_service(
            App::new()
                .wrap(Logger::default())
