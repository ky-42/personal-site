...
const checkUpdate = (baseContent: NewContent | Content, extraContent: NewBlog | NewProject | Blog | Project): {baseContent: Content, extraContent: Project | Blog} | undefined => {
  if ("id" in baseContent && "id" in extraContent) {
    return {baseContent, extraContent};
  }
}

const ContentForm = (action: actionTypes) => {
  ...
  const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (action === actionTypes.Create) {
      let newExtraContent: NewBlog | NewProject;
      switch (baseContent.content_type) {
        case "Blog":
          newExtraContent = blogContent;
        case "Project":
          newExtraContent = projectContent;
        default:
          newExtraContent = projectContent;
      }
      ...
      switch (baseContent.content_type) {
        case "Blog":
          extraContent = blogContent;
        case "Project":
          extraContent = projectContent;
        default:
          extraContent = projectContent;
      }
