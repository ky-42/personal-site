...
import {
  ...
  validateDevblog,
  validateProject,
} from '../components/ContentManagement/InputValidation';
...
const defaultExtraContent: ContentType = ContentType.Blog;
...
const contentReducer = <K extends keyof Content>(
  state: Content,
  action: SetReducer<Content> | UpdateReducer<Content, K>,
) => {
...
const blogReducer = <K extends keyof Blog>(
  state: Blog,
  action: SetReducer<Blog> | UpdateReducer<Blog, K>,
) => {
  switch (action.action) {
    case ReducerAction.Update: {
      const newState = { ...state };
...
const projectReducer = <K extends keyof Project>(
  state: Project,
  action: SetReducer<Project> | UpdateReducer<Project, K>,
) => {
  switch (action.action) {
    case ReducerAction.Update: {
      const newState = { ...state };
...
const devblogReducer = <K extends keyof Devblog>(
  state: Devblog,
  action: SetReducer<Devblog> | UpdateReducer<Devblog, K>,
) => {
  switch (action.action) {
    case ReducerAction.Update: {
      const newState = { ...state };
...
const ManageContent = () => {
  ...
  const [extraContentType, setExtraContentType] = useState<ContentType>(defaultExtraContent);
  ...
  const onSubmit = () => {
    ...
    switch (currentAction) {
      ...
      case ActionTypes.Delete: {
        const baseNewData = contentToNew(baseContentData);
        validateContent(baseNewData, newValidationErrors);
        const baseData = { ...baseContentData, ...baseNewData };
      ...
      case ActionTypes.DevblogDelete: {
        const devblogNewData = devblogToNew(devblogData);
  ...
  const DevblogManagerForm = (
    <InputSection>
      <SectionTitle>Devblog Data</SectionTitle>

      <DevblogManagement
        devblogData={devblogData}
        setDevblogData={setDevblogData}
        validationErrors={validationErrors}
      />
    </InputSection>
  );
    ...
    <ManageContentBody>
      ...
      <InputSection>
        ...
        <InputGroup>
          ...
          <InputGroup>
            <StateButton
              active={currentAction === ActionTypes.Create}
              onClick={() => setCurrentAction(ActionTypes.Create)}
            >
              Create
            </StateButton>
            <StateButton
              active={currentAction === ActionTypes.Update}
              onClick={() => setCurrentAction(ActionTypes.Update)}
            >
              Update
            </StateButton>
            <StateButton
              active={currentAction === ActionTypes.Delete}
              onClick={() => setCurrentAction(ActionTypes.Delete)}
            >
              Delete
            </StateButton>
          </InputGroup>
          ...
          <InputGroup>
            <StateButton
              active={currentAction === ActionTypes.DevblogCreate}
              onClick={() => setCurrentAction(ActionTypes.DevblogCreate)}
            >
              Create Devblog
            </StateButton>
            <StateButton
              active={currentAction === ActionTypes.DevblogUpdate}
              onClick={() => setCurrentAction(ActionTypes.DevblogUpdate)}
            >
              Update Devblog
            </StateButton>
            <StateButton
              active={currentAction === ActionTypes.DevblogDelete}
              onClick={() => setCurrentAction(ActionTypes.DevblogDelete)}
            >
              Delete Devblog
            </StateButton>
          </InputGroup>
              ...
              InputElement={
                <InputButtonHolder>
                  <ShortTextInput
                    type='text'
                    value={modifyItem}
                    onChange={(e) => setModifyItem(e.target.value)}
                  />
                  <EnterButton onClick={loadOldContent}>
                    <AiOutlineRight />
                  </EnterButton>
                </InputButtonHolder>
              }
